...
interface StatCardProps {
  stat: StatItem;
  onSelect: (stat: StatItem, type?: 'basic' | 'comprehensive' | 'advanced-cardnews') => void;
}
        ...
        {!showOptions ? (
          ...
          <div className="space-y-2 mt-4">
            ...
            <button 
              ...
              onClick={() => onSelect(stat, 'basic')}
            >
              <div className="flex items-center justify-between">
                <div>
                  <div className="font-medium">📊 기본 분석</div>
                  <div className="text-sm opacity-90">메타데이터 및 데이터 정리</div>
                </div>
                <div className="text-xs bg-blue-500 px-2 py-1 rounded">빠름</div>
              </div>
            </button>

            {/* 기본통계현황분석 */}
            <button 
              className="w-full bg-pink-600 text-white py-3 px-4 rounded-md hover:bg-pink-700 transition-colors text-left"
              onClick={() => onSelect(stat, 'advanced-cardnews')}
            >
              <div className="flex items-center justify-between">
                <div>
                  <div className="font-medium">📈 기본통계현황분석</div>
                  <div className="text-sm opacity-90">기초통계 지표 계산 및 현황 파악</div>
                </div>
                <div className="text-xs bg-pink-500 px-2 py-1 rounded">추천</div>
              </div>
            </button>
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-sm opacity-90">통계분석 + 트렌드 + 정책시사점 + 카드뉴스</div>
                </div>
